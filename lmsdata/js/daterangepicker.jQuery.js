(function ($) {
	...
	$.fn.daterangepicker = function(settings){
		...
		dateFormat: 'yymmdd', // date formatting. Available formats: http://docs.jquery.com/UI/Datepicker/%24.datepicker.formatDate
	...
	var datepickerOptions = {
		...
		$(document).ready(function() {
			var rpPresets = (function(){
				var ul = $('<ul class="ui-widget-content disnone"></ul>').appendTo(rp);
				...
				$.each(options.presets, function(key, value) {
					$('<li class="ui-daterangepicker-'+ key +' preset_'+ x +' ui-helper-clearfix ui-corner-all disnone"><span class="ui-icon ui-icon-triangle-1-e"></span><a href="#">'+ value +'</a></li>')
					.appendTo(ul);
					x++;
				});
		...
		// image button toggle rangepicker visibility
		$.each(options.buttonClasses, function(index, item) {
			$("."+item).click(function() {
				toggleRP();
				return false;
			})
